...
int check_strok_decrease(int a_i[M], size_t m);

void make_arrstrdec(int a[][M], int b[N], size_t n, size_t m);
...
int main()
{
    ...
    if (rc != EXIT_SUCCESS)
        return rc;
    make_arrstrdec(matrix, array, n, m);
    printf("Array: ");
    print_array(array, n);
    return rc;
}
